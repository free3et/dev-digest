...
- **DB columns & `@devdigest/shared` contract fields: `snake_case`**, and they
  mirror each other exactly — the wire JSON is never camelCased. E.g.
  `agent_runs.cost_usd` ↔ `RunStats.cost_usd` / `RunSummary.cost_usd`.
  `server/src/db/schema/runs.ts:21`, `server/src/vendor/shared/contracts/trace.ts:68`.
- **Drizzle schema fields: `camelCase`**, mapped to the snake_case column via
  the constructor's second argument — `costUsd: doublePrecision('cost_usd')`.
  Keep the TS field name and the SQL column name paired; never let a new
  column go in under a different case convention than its sibling fields.
- **Components: one `PascalCase` folder per component**, with colocated
  `styles.ts` / `constants.ts` / `helpers.ts` / `*.test.tsx` inside it — no
  bare `.tsx` files as siblings of a folder-based component.
- **i18n keys: `camelCase`, nested by screen section**
  (`list.*`, `timeline.*`, `verdict.*`, `panel.*` in
  `messages/<locale>/prReview.json`) — never a flat key list.
...
- Database migrations (`server/src/db/migrations/**`, `server/drizzle/**` if
  present) — do not invent or rewrite them unless a lesson explicitly says to.
