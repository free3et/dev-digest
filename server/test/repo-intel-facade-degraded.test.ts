...
describe('RepoIntel facade — degraded contract (flag off)', () => {
  ...
  it('getFileRank / getSymbolsInFiles / getConventionSamples / getConventionTestSamples / getTopFilesByRank / getCriticalPaths → []', async () => {
    const svc = buildDegradedService({ flag: false });
    ...
    await expect(svc.getConventionTestSamples('r1', 4)).resolves.toEqual([]);
